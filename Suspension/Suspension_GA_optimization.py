...
wheel_travel = 0.075  # Total wheel travel in m
...
c_initial = 0.20  # Lower arm length (NA)
...
def objective(params):
    ...
    for delta_z in delta_z_values:
        ...
        # Compute theta for motion ratios
        theta = np.arctan2(x_A_new - x_B_new, z_A_new - z_B_new)
        # Simplified motion ratios
        R_w = np.cos(theta)
        S_a = np.sin(theta) 
        S_b = -np.sin(theta)
    ...
    if not camber_angles:
        return 1e8
    camber_var = (np.max(camber_angles) - np.nanmin(camber_angles)) * 500
    camber_mean = np.nanmean(camber_angles)
    camber_mean_penalty = np.abs(camber_mean) * 5e12

    return camber_var + camber_mean_penalty + horizontal_penalty + parallelism_penalty + height_penalty
...
result = differential_evolution(objective, bounds, maxiter=1000, popsize=500)
